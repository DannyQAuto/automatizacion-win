...
console.log(`   📅 Fecha y hora solicitud: "${datos.FechaHoraRegistro}"`);
...
// Validación de código de solicitud
console.log(`🔍 Validando código de solicitud...`);
console.log(`   Extraído: "${codigoSolicitudEncontrado}"`);
console.log(`   Esperado: "${datos.CodigoSolicitud}"`);
expect(codigoSolicitudEncontrado).toBe(datos.CodigoSolicitud);
console.log('✅ CÓDIGO DE SOLICITUD VALIDADO CORRECTAMENTE');


// Validación de fecha de inicio
console.log(`🔍 Validando fecha de inicio...`);
console.log(`   Extraído: "${fechaInicioEncontrada}"`);
console.log(`   Esperado: "${datos.FechaInicio}"`);
expect(fechaInicioEncontrada).toBe(datos.FechaInicio);
console.log('✅ FECHA DE INICIO VALIDADA CORRECTAMENTE');

// Validación de fecha de fin
console.log(`🔍 Validando fecha de fin...`);
console.log(`   Extraído: "${fechaFinEncontrada}"`);
console.log(`   Esperado: "${datos.FechaFin}"`);
expect(fechaFinEncontrada).toBe(datos.FechaFin);
console.log('✅ FECHA DE FIN VALIDADA CORRECTAMENTE');

// Validación de días de suspensión
console.log(`🔍 Validando días de suspensión...`);
console.log(`   Extraído: "${diasSuspensionEncontrados}"`);
console.log(`   Esperado: "${datos.DiasSuspendidos}"`);
expect(diasSuspensionEncontrados).toBe(datos.DiasSuspendidos);
console.log('✅ DÍAS DE SUSPENSIÓN VALIDADOS CORRECTAMENTE');

// Validación de días disponibles
console.log(`🔍 Validando días disponibles...`);

// 1. Extraer número de días suspendidos del comentario (ej: "15 días" → 15)
const diasSuspendidosNum = parseInt(diasSuspensionEncontrados?.replace(/\D/g, '') || '0');

// 2. Calcular días disponibles esperados: 60 - días suspendidos
const diasDisponiblesEsperados = 60 - diasSuspendidosNum;

console.log(`   Días suspendidos del comentario: "${diasSuspensionEncontrados}"`);
console.log(`   Días suspendidos (número): ${diasSuspendidosNum}`);
console.log(`   Días disponibles calculados (60 - ${diasSuspendidosNum}): ${diasDisponiblesEsperados}`);
console.log(`   Días disponibles en JSON: "${datos.DiasDisponibles}"`);

// 3. Comparar (el JSON tiene "45" como string)
expect(diasDisponiblesEsperados.toString()).toBe(datos.DiasDisponibles);
console.log('✅ DÍAS DISPONIBLES VALIDADOS CORRECTAMENTE');


// Validación de fecha y hora de solicitud
console.log(`🔍 Validando fecha y hora de solicitud...`);
console.log(`   Extraído: "${FechayHoraSolicitud}"`);
console.log(`   Esperado: "${datos.FechaHoraRegistro}"`);
expect(FechayHoraSolicitud).toBe(datos.FechaHoraRegistro);
console.log('✅ FECHA Y HORA DE SOLICITUD VALIDADAS CORRECTAMENTE');
